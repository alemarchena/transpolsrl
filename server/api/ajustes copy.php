...
switch ($action) {
  ...
  case "insert":
    ...
    $sql = "INSERT INTO tajustes (idArticulo, tipo, cantidad, motivo, email)
            VALUES ($idArticulo, '$tipo', $cantidad, '$motivo', '$email')";
  ...
  case "update":
    ...
    $sql = "UPDATE tajustes
            SET idArticulo=$idArticulo, tipo='$tipo', cantidad=$cantidad, motivo='$motivo'
            WHERE id=$id";
    echo json_encode(["success" => $conn->query($sql)]);
    break;
