...
switch ($action) {
  case "get":
    ...
    // Total de personas
    $totalRes = $conn->query("SELECT COUNT(*) AS total FROM tpersonas");
    $total = ($totalRes && $totalRes->num_rows > 0) ? (int)$totalRes->fetch_assoc()["total"] : 0;

    // Consulta principal con LEFT JOIN para traer todas las personas
    $query = "
  ...
  case "descargarPDFsFirmados": // Nueva acción para descargar PDFs firmados como ZIP
    $anio = $conn->real_escape_string($data["anio"] ?? "");
    $mesInicio = (int)($data["mesInicio"] ?? 1);
    $mesFin = (int)($data["mesFin"] ?? 12);
    ...
    // Consulta para obtener solo los bonos firmados
    $query = "
