...
export default function Vehiculos() {
  ...
  const eliminarVehiculo = async (id) => {
    if (!(await Alerta.confirmar({ texto: "¿Deseas eliminar este vehículo ?" }))) return
    const res = await fetch(API_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
                ...
                <tr key={item.id} className="text-center border-t">
                  ...
                  <td className="p-2">
                    <button
                      ...
                      className="bg-yellow-500 text-white px-2 py-2 text-sm rounded"
                    >
                      Editar
                    </button>
                    <button
                      onClick={() => eliminarVehiculo(item.id)}
                      className="bg-red-600 text-white px-2 py-2 text-sm rounded ml-2"
                    >
                      <FaTrash />
                    </button>
                  </td>
